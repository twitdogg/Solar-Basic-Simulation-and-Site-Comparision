...
# Multi-step PV simulator: 1) Locations, 2) Electrical sizing, 3) Geometry & losses, 4) Run & Results
# Features: site search + CSV, full IANA timezone dropdown or auto, stringing overrides,
# pitch and auto table depth (from module geometry and tilt), slope + grading, polished Plotly charts.
...
/* Header */
.sv-hero {{
  background: linear-gradient(90deg, {PRIMARY} 0%, #0ea5a6 100%);
  color: white; padding: 18px 20px; border-radius: 14px; margin: 10px 0 16px 0;
}}
.sv-hero h1, .sv-hero h3, .sv-hero small {{ color: white !important; margin: 0; }}
/* Stepper */
.sv-stepper {{ display:flex; align-items:center; gap:10px; margin: 4px 0 16px 0; }}
.sv-step {{
  display:flex; align-items:center; gap:8px; padding:8px 12px; border-radius:999px; border:1px solid {BORDER};
  background:#fff; color:#111827; font-weight:600; font-size:14px;
}}
.sv-step.on {{ background:{PRIMARY}; color:white; border-color:{PRIMARY}; }}
.sv-dot {{
  width:22px; height:22px; line-height:22px; text-align:center; border-radius:50%;
  border:2px solid {PRIMARY}; color:{PRIMARY}; font-weight:700; background:white;
}}
.sv-step.on .sv-dot {{ background:white; color:{PRIMARY}; border-color:white; }}
/* Cards */
.sv-card {{
  background: {BG}; border: 1px solid {BORDER}; border-radius: 14px; padding: 16px; margin-bottom: 12px;
}}
.sv-title {{ font-weight: 700; font-size: 16px; margin-bottom: 8px; color:#111827; }}
/* Footer */
.sv-footer {{ text-align:center; color:#6B7280; padding:18px 0 8px 0; font-size: 13px; border-top: 1px solid {BORDER}; margin-top: 12px; }}
/* Metrics text size */
...
def grading_flag(slope_deg):
    if slope_deg is None:
        return "unknown", "Slope data unavailable. Sample nearby points or run a wider terrain study."
    if slope_deg < 3:
...
def stepper(current: int):
    labels = ["Locations", "Electrical sizing", "Geometry & losses", "Run & results"]
    cols = st.columns([1,1,1,1])
    for i, c in enumerate(cols, start=1):
        with c:
            on = "on" if i == current else ""
            st.markdown(
                f"""<div class="sv-step {on}">
                     <span class="sv-dot">{i}</span> {labels[i-1]}
                   </div>""",
...
def nav_buttons(show_back=True, show_next=True, next_label="Next"):
    ...
    back_clicked = next_clicked = False
    with cols[1]:
        if show_back:
...
if st.session_state.step == 1:
    with st.container():
        st.markdown('<div class="sv-card"><div class="sv-title">Add your sites</div>', unsafe_allow_html=True)
        tab1, tab2 = st.tabs(["Search or enter", "Upload / paste CSV"])

        with tab1:
            qcol, tcol, acol, pcol, dcol = st.columns([2,1,1,1,1])
            query = qcol.text_input("Search any city or 'lat,lon':", value="", help="Example: 'Calama, Chile' or '-23.65, -70.40'")
            default_tilt = tcol.number_input("Tilt (deg)", 0.0, 90.0, 15.0, 0.5, help="Plane tilt from horizontal")
            default_az   = acol.number_input("Azimuth (deg)", 0.0, 360.0, 180.0, 1.0, help="0=N, 90=E, 180=S, 270=W")
            default_pitch= pcol.number_input("Pitch (m)", 0.1, 100.0, 6.0, 0.1, help="Row-to-row spacing")
            default_depth= dcol.number_input("Table depth (m)", 0.1, 20.0, 2.1, 0.1, help="Front-to-back table footprint on ground")
            add = st.button("Add site", type="primary")
            if add:
                candidate = None
                # Try lat,lon direct
                try:
                ...
                if not candidate:
                    ...
                    row = {"name": candidate["name"], "lat": candidate["lat"], "lon": candidate["lon"],
                           "tz": tz, "tilt": default_tilt, "azimuth": default_az,
                           "pitch_m": default_pitch, "table_depth_m": default_depth}
                    st.session_state.sites_df = pd.concat([st.session_state.sites_df, pd.DataFrame([row])], ignore_index=True)
                    st.success(f"Added: {candidate['name']}  |  tz: {tz}")

        with tab2:
            st.caption("CSV columns allowed: name, lat, lon, tilt, azimuth, tz, pitch_m, table_depth_m. Missing tz will be auto-detected.")
            up = st.file_uploader("Upload CSV", type=["csv"])
            txt = st.text_area("...or paste CSV here")
            if st.button("Add from CSV"):
                try:
                    ...
                    if df is None:
                        ...
                        st.success(f"Added {len(added)} sites.")
                except Exception as e:
                    st.error(f"CSV error: {e}")

        # Editable table with full IANA dropdown
    ...
    # mini map to visually confirm positions
    try:
        map_df = st.session_state.sites_df.dropna(subset=["lat","lon"])[["lat","lon"]].copy()
        st.map(map_df, zoom=2)
    except Exception:
        pass

    back, next_ = nav_buttons(show_back=False, next_label="Continue to sizing")
    if next_:
        if st.session_state.sites_df.dropna(subset=["lat","lon"]).empty:
            st.error("Add at least one valid site first.")
        else:
...
# ------------------------------ STEP 2: Electrical sizing ------------------------------
    ...
    with st.container():
        ...
        st.session_state.gamma_pct_per_c = c2.number_input("Temp coefficient γPdc (%/°C)", -2.0, 0.0, st.session_state.gamma_pct_per_c, 0.01, help="Usually around -0.30% to -0.45% per °C")
        ...
        st.session_state.dc_ac_ratio_target = i3.number_input("Target DC/AC ratio (guidance)", 1.0, 2.5, st.session_state.get("dc_ac_ratio_target", 1.20), 0.01)

        # Live preview of derived counts
        module_w_stc = st.session_state.module_w_stc
        if st.session_state.override_total_strings:
            total_strings = int(st.session_state.total_strings_manual)
        else:
            total_strings = int(math.ceil((st.session_state.dc_kw_target * 1000.0) / (st.session_state.modules_per_string * module_w_stc)))

        if st.session_state.override_inverter_count:
            inverter_count = int(st.session_state.inverter_count_manual)
        else:
            inverter_count = int(math.ceil(total_strings / max(1, st.session_state.strings_per_inverter)))

        total_modules = total_strings * st.session_state.modules_per_string
        dc_w_actual   = total_modules * module_w_stc
        dc_kw_actual  = dc_w_actual / 1000.0
        ac_kw_nameplate = inverter_count * st.session_state.inv_kw_each
        dc_ac_ratio_actual = float("nan") if ac_kw_nameplate <= 0 else dc_kw_actual / ac_kw_nameplate

        m = st.columns(5)
        m[0].metric("Total strings", f"{total_strings}")
        m[1].metric("Modules/string", f"{st.session_state.modules_per_string}")
        m[2].metric("Inverters", f"{inverter_count}")
        m[3].metric("DC actual", f"{dc_kw_actual:,.2f} kWp")
        m[4].metric("AC nameplate", f"{ac_kw_nameplate:,.2f} kW")

        st.markdown('</div>', unsafe_allow_html=True)

    back, next_ = nav_buttons(next_label="Continue to geometry")
    if back:
        st.session_state.step = 1; st.rerun()
    if next_:
        st.session_state.step = 3; st.rerun()

# ------------------------------ STEP 3: Geometry & losses ------------------------------
elif st.session_state.step == 3:
    with st.container():
        st.markdown('<div class="sv-card"><div class="sv-title">Geometry and losses</div>', unsafe_allow_html=True)
        l1, l2, l3, l4 = st.columns(4)
        st.session_state.albedo   = l1.number_input("Albedo", 0.0, 1.0, st.session_state.albedo, 0.01)
        st.session_state.soiling  = l2.number_input("Soiling loss fraction", 0.0, 0.5, st.session_state.soiling, 0.01, help="Applied to AC output")
        st.session_state.dc_ohmic = l3.number_input("DC ohmic loss fraction", 0.0, 0.5, st.session_state.dc_ohmic, 0.01)
        st.caption("Per-site pitch and table depth can be edited on the Locations step. Table depth can be auto-calculated below.")

        st.markdown('<div class="sv-title" style="margin-top:4px;">Auto table depth from module geometry</div>', unsafe_allow_html=True)
        ad1, ad2, ad3, ad4 = st.columns(4)
        st.session_state.auto_depth = ad1.checkbox("Enable auto calculation", value=st.session_state.auto_depth, help="Uses tilt per site and inputs below")
    ...
    back, next_ = nav_buttons(next_label="Go to results")
    if back:
...
# ------------------------------ STEP 4: Run & results ------------------------------
    ...
    # Shortcuts to state
    sites_df = st.session_state.sites_df.dropna(subset=["lat","lon"]).copy()
    if sites_df.empty:
        st.error("No valid sites. Go back to Locations.")
    else:
        run = st.button("Run simulation", type="primary", use_container_width=True)
        if run:
            gamma = st.session_state.gamma_pct_per_c / 100.0
            module_w_stc = st.session_state.module_w_stc

            # stringing + capacity
            if st.session_state.override_total_strings:
                total_strings_actual = int(st.session_state.total_strings_manual)
            else:
                total_strings_actual = int(math.ceil((st.session_state.dc_kw_target * 1000.0) /
                                                     (st.session_state.modules_per_string * module_w_stc)))
            if st.session_state.override_inverter_count:
                inverter_count = int(st.session_state.inverter_count_manual)
            else:
                inverter_count = int(math.ceil(total_strings_actual / max(1, st.session_state.strings_per_inverter)))

            total_modules_actual = total_strings_actual * st.session_state.modules_per_string
            ...
            # KPIs display
            m = st.columns(5)
            m[0].metric("Total strings", f"{total_strings_actual}")
            m[1].metric("Modules/string", f"{st.session_state.modules_per_string}")
            m[2].metric("Inverters", f"{inverter_count}")
            m[3].metric("DC actual", f"{dc_kw_actual:,.2f} kWp")
            m[4].metric("AC nameplate", f"{ac_kw_nameplate:,.2f} kW")
            ...
            for _, r in sites_df.iterrows():
                ...
                # Auto or manual table depth
                if st.session_state.auto_depth:
                    stack = st.session_state.n_portrait * st.session_state.module_length_m + max(0, st.session_state.n_portrait - 1) * st.session_state.gap_vertical_m
                ...
                gcr = np.nan
                if pitch_m > 0:
                    gcr = max(0.0, min(1.0, table_depth_m / pitch_m))
                ...
                poa = irradiance.get_total_irradiance(
                    ...
                    albedo=st.session_state.albedo
                )
                poa_global = poa["poa_global"]

                # Temp and power
                wind = tmy["wind_speed"] if "wind_speed" in tmy.columns else None
                t_cell = cell_temperature_noct(
                    poa_global, tmy["temp_air"],
                    noct_c=st.session_state.noct_c,
                    wind_speed=wind,
                    wind_coeff=st.session_state.wind_coeff
                )
                ...
                pac_nom = pdc * st.session_state.inv_eff
                ac_w = np.minimum(pac_nom, ac_kw_nameplate * 1000.0) * (1 - st.session_state.soiling) * (1 - st.session_state.dc_ohmic)
            ...
            if not kpi_rows:
                st.error("No valid sites simulated.")
            else:
                # Results
                ...
                fig_month = px.bar(
                    ...
                    color_discrete_sequence=px.colors.qualitative.D3, template="plotly_white", height=440
                )
                ...
                fig_ann = px.bar(
                    ...
                    color="Specific yield (kWh/kWp)", color_continuous_scale="Tealrose",
                ...
                fig_slope = px.bar(
                    ...
                    color_discrete_map={"none":"#16a34a","minor":"#f59e0b","major":"#dc2626","unknown":"#6b7280"},
                    template="plotly_white", height=420
                )
    ...
    back, _ = nav_buttons(show_next=False)
    if back:
        st.session_state.step = 3; st.rerun()
